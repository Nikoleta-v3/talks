...
\begin{document}
	...
	\node[below right of=0] (minus_p1) {\textcolor{orange}D};

	\node[above right of=1] (p2) {\textcolor{cyan}C};
	\node[below right of=1] (minus_p2) {\textcolor{orange}D};

	\node[above right of=2] (p3) {\textcolor{cyan}C};
	\node[below right of=2] (minus_p3) {\textcolor{orange}D};

	\node[above right of=3] (p4) {\textcolor{cyan}C};
	\node[below right of=3] (minus_p4) {\textcolor{orange}D};

	\draw (0) edge[cyan, out=90, in=180, ->, thick] node [above] {$p_1$} (p1);
	\draw (0) edge[orange, out=-90, in=180, ->, thick] node [below] {$1-p_1$} (minus_p1);

	\draw (1) edge[cyan, out=90, in=180, ->, thick] node [above] {$p_2$} (p2);
	\draw (1) edge[orange, out=-90, in=180, ->, thick] node [below] {$1-p_2$} (minus_p2);

	\draw (2) edge[cyan, out=90, in=180, ->, thick] node [above] {$p_3$} (p3);
	\draw (2) edge[orange, out=-90, in=180, ->, thick] node [below] {$1-p_3$} (minus_p3);

	\draw (3) edge[cyan, out=90, in=180, ->, thick] node [above] {$p_4$} (p4);
	\draw (3) edge[orange, out=-90, in=180, ->, thick] node [below] {$1-p_4$} (minus_p4);
